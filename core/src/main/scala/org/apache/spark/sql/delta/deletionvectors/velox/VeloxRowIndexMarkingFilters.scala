...
import io.github.zhztheplayer.velox4j.memory.{BytesAllocationListener, MemoryManager}
...
object VeloxRowIndexMarkingFilters {
  ...
  abstract sealed class RowIndexMarkingFilters(bitmap: RoaringBitmapArray) extends RowIndexFilter {
    ...
    private val arrowAlloc: RootAllocator = new RootAllocator()
    private val veloxAllocListener: BytesAllocationListener = new BytesAllocationListener()
    private val memoryManager: MemoryManager = Velox4j.newMemoryManager(veloxAllocListener)
    private val session: Session = Velox4j.newSession(memoryManager)
